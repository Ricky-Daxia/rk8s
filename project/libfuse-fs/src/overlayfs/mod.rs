...
impl OverlayFs {
    ...
    async fn do_rename(
        ...
        let dest_node_opt = self.lookup_node_ignore_enoent(req, new_parent, new_name_str).await?;
        ...
        self.remove_inode(s_node.inode, s_node.path.read().await.clone().into()).await;
